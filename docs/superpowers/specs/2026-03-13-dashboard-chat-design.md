...
```json
{
  ...
  "history": [{"role": "user|spark", "text": "string"}]
}
```
Maximum 20 turns of history. Both limits enforced server-side before calling Claude.
...
In-memory dict keyed by IP (`request.client.host`). Sliding 10-minute window, max 10 messages per IP. Returns HTTP 429 with:
```json
{"error": "I'm still here — just need a moment before we keep going."}
```
...
- Current mood word (from latest thought in `state/thoughts.jsonl`)
- AEDT time of day
...
Conversation history is formatted as a dialogue block in the prompt body, not as separate API messages.

### Implementation File

`src/pxh/api.py` — add the `/api/v1/public/chat` endpoint and the in-memory rate limiter dict.
...
- Width: ~320px, max height: ~60vh with scroll
...
| Network error | "Something went quiet on my end. Try again?" |
...
### State Read (read-only)

The backend reads `state/thoughts.jsonl` and `state/awareness.json` to extract mood and weather for context injection. These are read-only accesses — no writes.

### Scaling Note

The in-memory rate limiter is not shared across worker processes. `api.py` is single-worker (`api.py` is not multi-worker safe per CLAUDE.md) so this is safe.
...
- **Sensory-friendly** — subtle animations only; no flashing, no jarring transitions, no sound
- **Autonomy-preserving** — no opener message, no pressure to respond, no timer or session expiry message
